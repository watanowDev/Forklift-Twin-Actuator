...
// PatliteLedBuzzer USB LED/Buzzer 제어를 위한 USB 통신 계층 구현
//
// libusb-1.0을 사용하여 USB 장치 직접 제어
// ============================================================================

#include "fta_actuators/patlite_led_buzzer/patlite_led_buzzer_usb_driver.hpp"
...
namespace fta_actuators
{
    namespace patlite_led_buzzer
    {
        ...
        PatliteLedBuzzerUsbDriver::PatliteLedBuzzerUsbDriver()
        ...
        bool PatliteLedBuzzerUsbDriver::open_device(uint16_t vendor_id, uint16_t product_id)
        {
            ...
            std::cout << "[PatliteLedBuzzerUsbDriver] Device opened successfully" << std::endl;
            return true;
        }

        void PatliteLedBuzzerUsbDriver::close_device()
        {
            if (device_handle_ != nullptr)
            {
            ...
            std::cout << "[PatliteLedBuzzerUsbDriver] Device closed" << std::endl;
        }

        bool PatliteLedBuzzerUsbDriver::is_connected() const
        {
        ...
        bool PatliteLedBuzzerUsbDriver::set_light(LEDColor color, LEDPattern pattern)
        {
            if (!is_connected())
            {
        ...
        bool PatliteLedBuzzerUsbDriver::set_buzzer(BuzzerPattern pattern, int volume, int count)
        {
            if (!is_connected())
            {
                set_error("Device not connected");
        ...
        bool PatliteLedBuzzerUsbDriver::get_device_state(bool &buzzer_active, bool &led_active, bool &touch_active)
        {
            if (!is_connected())
            {
                set_error("Device not connected");
        ...
        bool PatliteLedBuzzerUsbDriver::set_all(LEDColor color, LEDPattern led_pattern,
        ...
        // 모두 끄기
        // ============================================
        bool PatliteLedBuzzerUsbDriver::turn_off_all()
        {
            if (!is_connected())
            {
                set_error("Device not connected");
        ...
        UsbDeviceInfo PatliteLedBuzzerUsbDriver::get_device_info() const
        {
            return device_info_;
        }

        std::string PatliteLedBuzzerUsbDriver::get_last_error() const
        {
            return last_error_;
        }
        ...
        bool PatliteLedBuzzerUsbDriver::initialize_libusb()
        {
        ...
        bool PatliteLedBuzzerUsbDriver::find_and_claim_device(uint16_t vendor_id, uint16_t product_id)
        {
            ...
            if (!found)
            {
                set_error("PatliteLedBuzzer device not found (Vendor ID: 0x" +
        ...
        bool PatliteLedBuzzerUsbDriver::send_control_transfer(const uint8_t *data, int length)
        {
            // PatliteLedBuzzer는 Interrupt Transfer를 사용합니다 (OUT Endpoint 0x01)
            int transferred = 0;
            ...
            if (ret < 0)
            {
                set_error("Interrupt transfer failed: " + std::string(libusb_error_name(ret)));
                return false;
            }
        ...
        bool PatliteLedBuzzerUsbDriver::receive_control_transfer(uint8_t *data, int length)
        {
        ...
        void PatliteLedBuzzerUsbDriver::set_error(const std::string &error_msg)
        {
            last_error_ = error_msg;
            std::cerr << "[PatliteLedBuzzerUsbDriver ERROR] " << error_msg << std::endl;
        }
        ...
        std::vector<uint8_t> PatliteLedBuzzerUsbDriver::encode_led_command(LEDColor color, LEDPattern pattern)
        {
            ...
            std::cout << "[PatliteLedBuzzerUsbDriver] LED Command: color=" << static_cast<int>(color)
                      << " pattern=" << static_cast<int>(pattern) << std::endl;

            return data;
        }

        std::vector<uint8_t> PatliteLedBuzzerUsbDriver::encode_buzzer_command(BuzzerPattern pattern, int volume, int count)
        {
            // TODO: USB 패킷 캡처로 실제 프로토콜 분석 후 구현
            std::vector<uint8_t> data;
            ...
            std::cout << "[PatliteLedBuzzerUsbDriver] Buzzer Command: pattern=" << static_cast<int>(pattern)
                      << " volume=" << volume << " count=" << count << std::endl;

            return data;
        }

        std::vector<uint8_t> PatliteLedBuzzerUsbDriver::encode_combined_command(
            ...
            BuzzerPattern buzzer_pattern, int volume, int count)
        {
            ...
            std::cout << "[PatliteLedBuzzerUsbDriver] Combined Command: LED(color=" << static_cast<int>(color)
        ...
        {
            static PatliteLedBuzzerUsbDriver driver; // Singleton 패턴 (간단한 구현)
            ...
            if (!initialized)
            {
                ...
                {
                    std::cerr << "Failed to open PatliteLedBuzzer device: " << driver.get_last_error() << std::endl;
                    return false;
                }
                initialized = true;
            }
